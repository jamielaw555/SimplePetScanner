...
UmiPanoramaParameterisationBlocks::UmiPanoramaParameterisationBlocks( G4int nCopies, EnergyCounter * Counter ) :
  m_counter( Counter )
{
  ...
  for ( G4int copyNo = 0; copyNo < nCopies; ++copyNo )
  {
    ...
    G4double const r = 38.905 * cm; // 76cm "Detector ring diameter
    // Set the translation
...
void UmiPanoramaParameterisationBlocks::ComputeTransformation( const G4int copyNo,  G4VPhysicalVolume* physVol ) const
{
  if ( copyNo >= ( G4int )m_positions.size() )
  {
    G4cerr << "Unknown copyNo for UmiPanoramaParameterisationBlocks: " << copyNo << G4endl;
    return;
  }
