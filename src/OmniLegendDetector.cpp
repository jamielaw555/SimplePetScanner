...
#include "OmniLegendParameterisationBlocks.h"
#include "OmniLegendParameterisationPanels.h"
...
G4VPhysicalVolume* OmniLegendDetector::Construct(std::string Name, G4LogicalVolume* worldLV, std::string Mode, EnergyCounter* Counter, G4double LengthMM, std::string Material)
{
    // Default length
    if (LengthMM <= 0.0)
    {
        LengthMM = 1280.0; // this is without considering the effect of gaps
    }

    G4NistManager* nistManager = G4NistManager::Instance();
    G4Material* air = nistManager->FindOrBuildMaterial("G4_AIR");
    G4Material* crystal = CrystalMaterial::GetCrystalMaterial(Material, "Legend");
    ...
    if (nRings == 24)
    {
        std::cout << "Omni Legend detector with nRings: " << nRings << std::endl;
    }
    else
    {
        std::cout << "Omni Legend detector variant with nRings: " << nRings << std::endl;
    }
    ...
    G4double const envelopeInnerRadius = 36.27 * cm; // 725.4
    ...
    G4int blocksPerRing = 22;
    if (Mode == "Crystal")
    {
        G4VPVParameterisation* detectorParam = new OmniLegendParameterisationCrystals(288 * blocksPerRing * nRings);
        return new G4PVParameterised(Name, detectorLV, envelopeLV, kUndefined, 288 * blocksPerRing * nRings, detectorParam);
    }
    else if (Mode == "Block")
    {
        G4VPVParameterisation* detectorParam = new OmniLegendParameterisationBlocks(blocksPerRing * nRings, nullptr);
    ...
    {
        G4VPVParameterisation* detectorParam = new OmniLegendParameterisationBlocks(blocksPerRing * nRings, Counter);
        return new G4PVParameterised(Name, detectorLV, envelopeLV, kUndefined, blocksPerRing * nRings, detectorParam);
    }
    else if (Mode == "Panel")
    {
        G4VPVParameterisation* detectorParam = new OmniLegendParameterisationPanels(blocksPerRing);
        return new G4PVParameterised(Name, detectorLV, envelopeLV, kUndefined, blocksPerRing, detectorParam);
    }
    else
    {
        std::cerr << "Unrecognised Omni Legend detector mode: " << Mode << std::endl;
        exit(1);
    }
}

G4int OmniLegendDetector::NRingsInLength(G4double const LengthMM)
{
    return ceil(LengthMM * mm / (blockAxial * 2.0));
}

G4double OmniLegendDetector::LengthForNRings(G4int const NRings)
{
