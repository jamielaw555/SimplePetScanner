...
#include "UmiPanoramaParameterisationCrystals.h"
#include "UmiPanoramaParameterisationBlocks.h"
#include "UmiPanoramaParameterisationPanels.h"
...
G4VPhysicalVolume* UmiPanoramaDetector::Construct( std::string Name, G4LogicalVolume* worldLV, std::string Mode, EnergyCounter * Counter, G4double LengthMM, std::string Material )
{
  // Default length
  if ( LengthMM <= 0.0 )
  {
  ...
  G4Material* crystal = CrystalMaterial::GetCrystalMaterial(Material, "Panorama");
  ...
  G4double const envelopeInnerRadius = 36.0 * cm;
  ...
  G4int blocksPerRing = 34;
  if ( Mode == "Crystal" )
  {
    G4VPVParameterisation* detectorParam = new UmiPanoramaParameterisationCrystals( 576*blocksPerRing*nRings );
    return new G4PVParameterised( Name, detectorLV, envelopeLV, kUndefined, 576*blocksPerRing*nRings, detectorParam );
  }
  else if ( Mode == "Block" )
  {
    G4VPVParameterisation* detectorParam = new UmiPanoramaParameterisationBlocks( blocksPerRing*nRings, nullptr );
  ...
  {
    G4VPVParameterisation* detectorParam = new UmiPanoramaParameterisationBlocks( blocksPerRing*nRings, Counter );
    return new G4PVParameterised( Name, detectorLV, envelopeLV, kUndefined, blocksPerRing*nRings, detectorParam );
  }
  else if ( Mode == "Panel" )
  {
    G4VPVParameterisation* detectorParam = new UmiPanoramaParameterisationPanels( blocksPerRing );
    return new G4PVParameterised( Name, detectorLV, envelopeLV, kUndefined, blocksPerRing, detectorParam );
  }
  else
  {
    std::cerr << "Unrecognised Panorama Quadra detector mode: " << Mode << std::endl;
    exit(1);
  }
}

G4int UmiPanoramaDetector::NRingsInLength( G4double const LengthMM )
{
  return ceil( LengthMM * mm / ( blockAxial * 2.0 ) );
}
G4double UmiPanoramaDetector::LengthForNRings( G4int const NRings )
{
