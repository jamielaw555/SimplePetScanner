...
UmiPanoramaParameterisationCrystals::UmiPanoramaParameterisationCrystals( G4int nCopies )
{
  ...
  for ( G4int copyNo = 0; copyNo < nCopies; ++copyNo )
  {
    ...
    G4int const crystalsPerRing = 19584;
    // 7 blocks (cells) axially and 34 transaxially
    G4int const blocksPerRing = 34;
    ...
    G4int const crystalsPerBlock = 576;
    ...
    G4int const crystalsBlockAxial = 24;
    ...
    G4double const r = 38.905 * cm; // 76cm "Detector ring diameter" "Transaxial FOV"
...
void UmiPanoramaParameterisationCrystals::ComputeTransformation( const G4int copyNo, G4VPhysicalVolume* physVol ) const
{
  if ( copyNo >= ( G4int )m_positions.size() )
  {
    G4cerr << "Unknown copyNo for UmiPanoramaParameterisationCrystals: " << copyNo << G4endl;
    return;
  }
